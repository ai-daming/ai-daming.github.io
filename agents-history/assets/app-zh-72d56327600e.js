function createEntityUI(config){
 ...
 function renderCatalog(q=query){
  ...
  const head=el('header','catalog-heading');head.append(el('h1','','沿着一个对象，看它怎样演化'),el('p','','产品、模型、协议、方法与研究，共用同一批月度事件。当前资料范围：2024.01 — 2026.09.21。'));
  catalog.append(head);
 ...
 function renderEntity(id){
  ...
  sourcePanel.append(el('h2','','官方更新日志与历史来源'),el('p','meta',e.changelogCount+' 条已收录官方更新 · 资料截至 2026-09-21'));
...
(() => {
 ...
 function fieldSection(m,f,title,prefix='sec'){
  ...
  if(f==='discussion')m.eventIds.map(id=>byEvent.get(id)).filter(e=>e.kind==='匿名社群讨论').forEach(e=>s.append(record(e)));
  return s;
 }
 function record(e){
  const d=el('details','record');d.dataset.record=e.id;if(e.isChangelog)d.classList.add('changelog-record');const sum=el('summary');const body=e.body.replace(/\s+/g,' '),short=e.kind==='匿名社群讨论'?e.body.split('\n')[0]:(body.length>84?body.slice(0,84)+'…':body);
 ...
 function renderDetail(m){
  ...
  reading.replaceChildren();const head=el('div','reader-heading');head.append(el('h1','',m.id.slice(0,4)+' 年 '+Number(m.id.slice(5))+' 月'),el('p','',m.id==='2026-09'?'本月资料补至 9 月 21 日':'月份不跳过，资料逐条展开'));if(m.changelogCount)head.append(btn(m.changelogCount+' 条官方更新记录',()=>goSection('events'),'changelog-jump'));reading.append(head);
