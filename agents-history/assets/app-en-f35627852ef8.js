function createEntityUI(config){
 ...
 function renderCatalog(q=query){
  ...
  const head=el('header','catalog-heading');head.append(el('h1','',"Follow an object through its history"),el('p','',"Products, models, protocols, methods and research share the same monthly events. Current coverage: January 2024 – September 21, 2026."));
  catalog.append(head);
 ...
 function renderEntity(id){
  ...
  sourcePanel.append(el('h2','',"Official updates & historical sources"),el('p','meta',e.changelogCount+" recorded official updates · Through 2026-09-21"));
  ...
  sourcePanel.append(el('p','meta',"Version records retain their dates and original sources. Topic labels are an index, not claims of new capabilities or independently verified results."));
...
(() => {
 ...
 function renderOverview(){
  ...
  months.forEach(m=>{
   ...
   n.append(el('h2','',m.id.replace('-','.')));if(m.changelogCount)n.append(el('p','changelog-count',m.changelogCount+" official update records"));
 ...
 function fieldSection(m,f,title,prefix='sec'){
  const s=el('section','section');s.id=prefix+'-'+f;s.dataset.field=f;s.append(el('h2','',title),rich(el('p'),m[f],m.id));
  if(f==='discussion')m.eventIds.map(id=>byEvent.get(id)).filter(e=>e.kind==="Anonymous community discussion").forEach(e=>s.append(record(e)));
  return s;
 }
 function record(e){
  const d=el('details','record');d.dataset.record=e.id;if(e.isChangelog)d.classList.add('changelog-record');const sum=el('summary');const body=e.body.replace(/\s+/g,' '),short=e.kind==="Anonymous community discussion"?e.body.split('\n')[0]:(body.length>84?body.slice(0,84)+'…':body);
 ...
 function renderDetail(m){
  ...
  reading.replaceChildren();const head=el('div','reader-heading');head.append(el('h1','',m.id.slice(0,4)+" / "+Number(m.id.slice(5))+""),el('p','',m.id==='2026-09'?"This month is covered through September 21":"Every month, with detailed records and sources"));if(m.changelogCount)head.append(btn(m.changelogCount+" official update records",()=>goSection('events'),'changelog-jump'));reading.append(head);
