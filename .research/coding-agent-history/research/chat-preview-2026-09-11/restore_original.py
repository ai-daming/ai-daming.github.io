...
for route,rel in list(routes.items()):
 ...
 source=(SITE/rel).read_text();m=re.search(r'<div id="static-content">(.*?)</div>\s*<main id="main">',source,re.S)
 assert m,route
 target=OUT/'reading'/rel;target.parent.mkdir(parents=True,exist_ok=True);target.write_text('<!doctype html><html><head><meta charset="utf-8"><title>阅读资料</title></head><body><main id="paper">'+m[1]+'</main></body></html>')
 routes['/reading'+route]='reading/'+rel
for f in manifest['files']:
 if f.startswith('assets/'):routes['/agents-history/'+f]='agents-history/'+f
for name in ['overlay.js','overlay.css','marked.umd.js','markdown.js','marked-LICENSE.md','markdown.css','context.js','context.css']:
