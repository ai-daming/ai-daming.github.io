...
## Market evidence track
Implementation Gate: READY (delta at 170cc44). User accepted a lower track aligned to disclosure months, independent usage bars, blank gaps and expandable methods/source evidence. Reuse the original film and navigation as the sole position owner; read actual column geometry and scroll offset without replacing its event handlers. An isolated plugin reserves vertical room only in overview and translates the lower cells to match. On mobile, the lower track shows the active month of the vertical timeline. Other views retain their height and hide the track. Data owner is the previously researched sample.json (21 observations); only 19 with known disclosure month enter eight positions. Two unknown-date Menlo records remain in a separate pending-methods panel. Build emits readable HTML, not a market JSON/data bundle. Percent bars use independent 0–100 scales; revenue/users are milestone labels, never normalized percentages. Modal explains windows, source/bias and conditional cross-wave comparison. Chat and original page assets remain unchanged; local preview only, no commit/push/deploy authority. Verify positions through animation, gaps, 2026-04/08 values, detail sources, mobile/theme, and unknown-date exclusion.

Market track default-collapse correction — Implementation Gate: READY (delta). User explicitly requests a collapsed default with manual expansion. One transient boolean controls the track viewport, aria-expanded, and the reserved vertical space (32px collapsed; prior height expanded). The original timeline remains the scroll/navigation owner. No persistence, data, route, backend or publication change. Existing open pages can receive the collapse control without restarting Chat.

## Approved production release
Implementation Gate: READY (delta, baseline 170cc447affc32a30617a8c651606249a6f755f3). User now explicitly authorizes publication to daming.ai, followed by commit and push. Preserve the confirmed original UI, memory-only BYOK Chat, Markdown renderer and collapsed market track. Move all added resources/readers under /agents-history/, handled by the existing topic Worker; do not alter root Worker routing. Add noindex/canonical metadata to reader copies. Publish readable market HTML only; do not publish the private market dataset. Core existing data loading stays unchanged, as previously disclosed. Use an explicit public-file allowlist, verify byte hashes/routes/privacy before deployment, record the Cloudflare version, verify HTTP/UI afterward, then commit/push only release source and public files. Build strips previous addon markers for idempotence; a clean checkout can reuse already-published readable market HTML if the private research input is absent. Failed checks block deployment; uncertain deployment is inspected before retry; prior Worker version provides rollback. No key discovery or model call is needed for deployment.
